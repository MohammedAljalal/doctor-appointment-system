...
const MyAppointment = () => {
  ...
  const getUserAppointment=async ()=>{
    try {
      const {data}=await axios.get(backendUrl + '/api/user/listAppoitment',{ headers: {
          Authorization: token
        }})
    ...
    setUserAppointment([...data.data].reverse())
}
  ...
  return (
    <div>
      <p className='pb-3 mt-12 font-medium text-zinc-700 border-b border-b-gray-200'>My appointments</p>
      <div>
        {
        

          userAppointment.slice(0,5).map((item,index)=>(
            ...
            <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b border-b-gray-200' key={index}>
              ...
              <div className='flex-1 text-sm text-zinc-600'>
                ...
                <p className='text-sm mt-1 '><span className='text-sm text-neutral-700 font-medium' >Date && Time:</span>{slotDateFormat(item.slotDate)} | {formatTime(item.slotTime)}</p>
                ...
                <div className='flex flex-col gap-2 justify-end'>
                  ...
                  {!item.cancelled && item.isCompleted && <button className='sm:min-w-48 py-2 border  border-green-500 rounded text-green-500  '>Appointment Completed</button> }
                  {
