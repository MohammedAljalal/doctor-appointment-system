 ...
 const address = (() => {
  ...
  if (typeof userData.address === "string") {
    try {
      ...
      console.warn("Address is not valid JSON:", userData.address);
      return {};
    }
  }
  ...
  <p className='text-gray-500'>
    ...
    {address.line2}
     ...
     <p className='font-medium'>Birthday:</p>
       ...
       {
      isEdit ? <input className='max-w-28 bg-gray-100' type="date" onChange={(e)=>setUserData(prev=>({...prev,date_Of_Birth:e.target.value}))} value={userData.date_Of_Birth} />
     ...
     <div className='mt-10 '>
      {
        isEdit ? <button className='border border-primary px-8 py-2 rounded-full hover:bg-primary hover:text-white transition-all ' onClick={updateUserProfileDAta}>Save Information</button>
                : <button className='border border-primary px-8 py-2 rounded-full hover:bg-primary hover:text-white transition-all  ' onClick={()=>setIsIdit(true)}>Edit</button>}
     </div>
